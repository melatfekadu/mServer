...
module.exports.controller = function (app)  {
 ...
 app.get('/employees', (req, res) => {
  Employee.find({}, 'emp_id first_name last_name email phone_no gender department user_name password', function (error, employees) {
...
app.get('/employee/:id', (req, res) => {
  Employee.findById(req.params.id, 'emp_id first_name last_name email phone_no gender department user_name password', function (error, employee) {
  if (error) { console.log(error); }
  res.send(employee)
 })
})
app.get('/employees/isIdTaken/', (req, res) => {
  Employee.countDocuments({ email: req.query.email }, function (error, customerCount) {
  if (error) { console.log(error); }
  //customerCount= 3;
  res.json({ 
